...
public class SimplePageContent<D> implements PageContent<D> {
    ...
    public ItemStack getItemStack(GuiInfo guiInfo, D data) {
        return MessageUtil.format(languageManager != null ? languageManager.getLanguage((Player) guiInfo.getEntity()) : null, itemStackFunction.apply(setting, guiInfo, data), messageValues.stream().map(function -> function.apply(setting, guiInfo, data)).toArray(MessageValue[]::new));
    }
